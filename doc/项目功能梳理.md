...
| `AudioLoopPracticeFragment` | Fragment | `feature-learning` | `PracticeActivity` 音频循环模式 | 设置弹窗、页内播放器 |
...
| 音频循环 | 页面内播放器顺序播放单词与例句，支持展示开关与播放策略设置 | `AudioLoopPracticeFragment`、`AudioLoopSettingsDialogFragment` | `PracticeActivity` | 页内播放控制、设置保存、播放列表跳转 |
...
#### 11. 音频循环 `AudioLoopPracticeFragment`

| 维度 | 说明 |
|---|---|
| 作用 | 在页面内顺序播放单词与例句，适合碎片化听记。 |
| 对应页面 / 状态 | 音频循环页、设置弹窗。 |
| 入口 | `PracticeActivity` 音频循环模式。 |
| 出口 | 退出页面或切换到其他练习模式。 |
| 核心业务逻辑 | 设置中可选择练习模式、播放次数、播放间隔、是否循环，以及是否展示拼读/中文；页面内播放器按顺序播单词与例句，到末尾时按 `loopEnabled` 决定停止或重头开始。 |
| 前置条件 | 有词可播。 |
| 限制条件 | 依赖页面内播放器；不是问答式会话。 |
| 异常分支 | 无词时无法启动；音频失败会自动跳过当前条目并在整轮失败时停播。 |
| 关键交互 | 播放/暂停、上一项、下一项、播放列表、打开设置。 |
