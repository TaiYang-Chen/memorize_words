...
@Entity(
    ...
    indices = [
        Index("word_id"),
        Index("book_id"),
        Index("next_review_at_ms"),
        Index(value = ["book_id", "user_status"]),
        Index(value = ["book_id", "next_review_at_ms"]),
        Index(value = ["book_id", "user_status", "next_review_at_ms", "last_learned_at_ms", "word_id"])
    ],
)
data class WordLearningStateEntity(
    ...
    /** 最近一次学习时间 */
    @ColumnInfo(name = "last_learned_at_ms")
    val lastLearnedAtMs: Long,

    /** 下次复习时间（记忆曲线） */
    @ColumnInfo(name = "next_review_at_ms")
    ...
    init {
        ...
        require(lastLearnedAtMs >= 0L) { "lastLearnedAtMs must be non-negative" }
        require(nextReviewAtMs >= 0L) { "nextReviewAtMs must be non-negative" }
