...
@HiltViewModel
class EmailCodeLoginViewModel @Inject constructor(
    ...
    fun sendCode() {
        ...
        viewModelScope.launch {
            sendEmailCodeUseCase(targetEmail, scene = LOGIN_EMAIL_SCENE).onSuccess { meta ->
    ...
    private fun login(cancelDeletion: Boolean) {
        ...
        launchWithLoading(resourceProvider.getString(R.string.module_user_login_loading)) {
            emailCodeLoginUseCase(targetEmail, code, cancelDeletion).onSuccess {
    ...
    companion object {
        const val ACTION_CANCEL_DELETION_LOGIN = "email_code_cancel_deletion_login"
        private const val LOGIN_EMAIL_SCENE = "login"
